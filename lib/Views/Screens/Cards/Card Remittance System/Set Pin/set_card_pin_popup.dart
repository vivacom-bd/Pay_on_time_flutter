...
class DialogForSetPin extends StatelessWidget {
  const DialogForSetPin({
    Key? key,
  }) : super(key: key);
            ...
            DefaultButton(
              ...
              onTap: () async {
                  if(pinController.text.isNotEmpty){
                    ...
                    bool value = await commonController.cardPinSet(commonController.personalAccountCard.value.data![0].id!, pinController.text);
                    Get.back();
                    if(value){
                      Utility.showSnackBar(commonController.pinSet.value.reason!);
                      Get.to(const SetPinSuccessfulLoadingScreen());
                    }
